...
const { startExpiryJob, runExpiryCheck } = require("./jobs/expiryCheckJob");
...
app.post("/api/expiry/run", authenticate, async (_req, res) => {
  try {
    await runExpiryCheck();
    res.json({ success: true, message: "Expiry check completed" });
  } catch (err) {
    console.error("❌ Expiry run error:", err.message);
    res.status(500).json({ success: false, message: err.message });
  }
});
